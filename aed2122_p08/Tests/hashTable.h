...
bool HashTable<KeyType>::insert(const KeyType& k) {
    ...
    if (numEmpty - 1 <= 0 || (table[i].state != EMPTY && table[i].key == k )) return false;
...
bool HashTable<KeyType>::remove(const KeyType& k) {
    ...
    if (!(table[i].key == k && table[i].state == ACTIVE)) return false;
...
bool HashTable<KeyType>::rehash(int n) {
    ...
    vector<HashEntry> copy = table;

    vector<HashEntry> newHashTable(n);

    table = newHashTable;

    for (auto ele : copy) {
        if (ele.state == ACTIVE) insert(ele.key);
    }
