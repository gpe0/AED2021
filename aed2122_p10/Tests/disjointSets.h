...
T DisjointSets<T>::find(const T& x) {
    if (a[x].parent != x) a[x].parent = find(a[x].parent);
...
void DisjointSets<T>::unite(const T& x, const T& y) {
    ...
    if (a[xRoot].myrank < a[yRoot].myrank) a[xRoot].parent = yRoot;

    else if (a[xRoot].myrank > a[yRoot].myrank) a[yRoot].parent = xRoot;
